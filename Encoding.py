...
class DeterministicRate(Encoder):
    ...
    def encode(self, data):
        ...
        # Salida esperada: torch.Size([num_steps, 128, 1, 28, 28])
       
        spikes_list = []
       
        # Inicializamos la memoria con ceros del mismo tamaño que el input
        if self.v_mem is None:
            self.v_mem = torch.zeros_like(data)
        ...
        for t in range(self.num_steps):
            ...
            # 2. Comprobar disparo (Threshold = 1.0)
            spike = (self.v_mem >= 1.0).float()
           
            # 3. Restar el umbral (Soft reset) solo donde hubo spike
            # Si hubo spike (1), restamos 1. Si no (0), restamos 0.
            self.v_mem -= spike
