...
class SNNExperiment:
    def __init__(self, config):
        ...
        self.trainer = Trainer(self.net, self.optimizer, nn.CrossEntropyLoss(), self.device)
    ...
    def run(self):
        ...
        for epoch in range(self.config['num_epochs']):
            ...
            self.trainer.train_epoch(train_loader, self.encoder, self.config['num_steps'])

            if (epoch + 1) % self.config['eval_freq'] == 0:
                accuracy = self.trainer.evaluate(test_loader, self.encoder, self.decoder, self.config['num_steps'])
